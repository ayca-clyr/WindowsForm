...
namespace Part_1
{
    public partial class ProductEditForm : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (btnAdd.Text == "Add")
            {
                proListForm.MdiParent = Form1.ActiveForm;
                db.Database.ExecuteSqlCommand("insert into Categories (ProductName,CategoryID,SupplierID,UnitsInStock,UnitPrice) values(@proName,@catID,@supID,@stock,@price)", new SqlParameter("@ad", txtBxProductName.Text), new SqlParameter("@catID", (cBxCategoryID.SelectedIndex + 1).ToString()), new SqlParameter("@supID", (cBxSupplierID.SelectedIndex + 1).ToString()), new SqlParameter("@stock", int.Parse(txtBxStock.Text)), new SqlParameter("@price", decimal.Parse(txtBxPrice.Text)));
